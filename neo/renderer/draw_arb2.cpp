...
void	RB_ARB2_DrawInteraction( const drawInteraction_t *din ) {
	// GLSL path: matrice MVP calculee dans le bon ordre (modelView, projection) - CONFIRME FONCTIONNEL
	float mvp[16];
	myGlMultMatrix( din->surf->space->modelViewMatrix, backEnd.viewDef->projectionMatrix, mvp );
	qglUniformMatrix4fv( interactionProg.loc_modelViewProj, 1, GL_FALSE, mvp );

	qglUniform4fv( interactionProg.loc_lightOrigin,    1, din->localLightOrigin.ToFloatPtr() );
	qglUniform4fv( interactionProg.loc_viewOrigin,     1, din->localViewOrigin.ToFloatPtr() );
	qglUniform4fv( interactionProg.loc_lightProjS,     1, din->lightProjection[0].ToFloatPtr() );
	qglUniform4fv( interactionProg.loc_lightProjT,     1, din->lightProjection[1].ToFloatPtr() );
	qglUniform4fv( interactionProg.loc_lightProjQ,     1, din->lightProjection[2].ToFloatPtr() );
	qglUniform4fv( interactionProg.loc_lightFalloffS,  1, din->lightProjection[3].ToFloatPtr() );
	qglUniform4fv( interactionProg.loc_bumpMatrixS,    1, din->bumpMatrix[0].ToFloatPtr() );
	qglUniform4fv( interactionProg.loc_bumpMatrixT,    1, din->bumpMatrix[1].ToFloatPtr() );
	qglUniform4fv( interactionProg.loc_diffuseMatrixS, 1, din->diffuseMatrix[0].ToFloatPtr() );
	qglUniform4fv( interactionProg.loc_diffuseMatrixT, 1, din->diffuseMatrix[1].ToFloatPtr() );
	qglUniform4fv( interactionProg.loc_specMatrixS,    1, din->specularMatrix[0].ToFloatPtr() );
	qglUniform4fv( interactionProg.loc_specMatrixT,    1, din->specularMatrix[1].ToFloatPtr() );

	static const float zero[4] = { 0, 0, 0, 0 };
	static const float one[4] = { 1, 1, 1, 1 };
	static const float negOne[4] = { -1, -1, -1, -1 };

	// GLSL path: v_color via qglUniform4fv - CONFIRME FONCTIONNEL (bug precedent: ARB assembly au lieu de GLSL)
	switch ( din->vertexColor ) {
	case SVC_IGNORE:
		qglUniform4fv( interactionProg.loc_colorModulate, 1, zero );
		qglUniform4fv( interactionProg.loc_colorAdd, 1, one );
		break;
	case SVC_MODULATE:
		qglUniform4fv( interactionProg.loc_colorModulate, 1, one );
		qglUniform4fv( interactionProg.loc_colorAdd, 1, zero );
		break;
	case SVC_INVERSE_MODULATE:
		qglUniform4fv( interactionProg.loc_colorModulate, 1, negOne );
		qglUniform4fv( interactionProg.loc_colorAdd, 1, one );
		break;
	}

	qglUniform4fv( interactionProg.loc_diffuseModifier, 1, din->diffuseColor.ToFloatPtr() );
	qglUniform4fv( interactionProg.loc_specularModifier, 1, din->specularColor.ToFloatPtr() );
...
void RB_ARB2_CreateDrawInteractions( const drawSurf_t *surf ) {
	...
	// GLSL path: un seul program combine vertex+fragment - CONFIRME FONCTIONNEL
	qglUseProgram( interactionProg.program );

	// enable the vertex arrays - 0 (position) et 1 (couleur) EN PLUS de 8,9,10,11
	// (le bug de l'ecran noir venait d'un oubli de qglEnableVertexAttribArray(0))
	...
	for ( ; surf ; surf=surf->nextOnLight ) {
		...
		// set the vertex pointers - attributs generiques 0 et 1 au lieu du fixed-function
